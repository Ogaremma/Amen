...
from app.services.sportybet import _draw_selection, create_draw_booking, get_upcoming_football_events, parse_upcoming_events_page
...
def _local_sportybet_events(snapshot_dir: Path | None = None):
    ...
    total_num = 0
    for _, path in files:
        parsed = parse_upcoming_events_page(json.loads(path.read_text(encoding="utf-8")))
        total_num = max(total_num, parsed.total_num)
        events.extend(parsed.events)
    return type("SnapshotEvents", (), {"events": events, "total_num": total_num, "pages_fetched": len(files)})()

async def process_snapshot(request: ForebetAcquisitionSnapshotRequest, *, execute_booking: bool = False, engine: ForebetDrawEngine | None = None) -> dict:
    ...
    if trusted_mode and execute_booking:
        raise ValueError("trusted snapshot processing is paper-only; real booking is disabled")
    supplied_dates = [snapshot.prediction_date for snapshot in request.snapshots]
    if trusted_mode and (len(request.snapshots) != 3 or supplied_dates != rolling_dates):
        raise ValueError(f"snapshots must contain exactly the current rolling dates in order: {', '.join(map(str, rolling_dates))}")
    for snapshot in request.snapshots:
        if trusted_mode and snapshot.raw_html is None:
            raise ValueError(f"trusted snapshot raw_html is required for {snapshot.prediction_date}")
        url_match = re.search(r"/(\d{4}-\d{2}-\d{2})(?:[/?#]|$)", snapshot.source_url)
        if not url_match or date.fromisoformat(url_match.group(1)) != snapshot.prediction_date:
            raise ValueError(f"snapshot source_url date does not match prediction_date: {snapshot.prediction_date}")
    normalized = []
    for snapshot in request.snapshots:
        if snapshot.raw_html is not None:
            lowered = snapshot.raw_html.lower()
            if '<html' not in lowered or 'schema' not in lowered:
                raise ValueError(f"PARSER_FAILURE: invalid Forebet HTML for {snapshot.prediction_date}")
            parsed = parse_forebet_html(snapshot.raw_html, snapshot.source_url)
            dated = [m for m in parsed if m.kickoff is not None]
            normalized.append(snapshot.model_copy(update={"matches": [m for m in dated if _kickoff_lagos_date(m.kickoff) == snapshot.prediction_date]}))
            forebet_draw_store.save_raw_snapshot(snapshot.prediction_date, snapshot.source_url, snapshot.raw_html, source=snapshot.source)
        else:
    ...
    if request.sportybet_events is not None:
        ...
        events = [event for event in request.sportybet_events if event.sport_id == settings.sportybet_football_sport_id and event.kickoff.date() in target_dates]
        upcoming = type("SnapshotEvents", (), {"events": events})()
    elif request.use_local_sportybet_snapshots:
        local = _local_sportybet_events()
        events = [event for event in local.events if event.sport_id == settings.sportybet_football_sport_id and event.kickoff.date() in allowed_dates]
        upcoming = type("SnapshotEvents", (), {"events": events, "total_num": local.total_num, "pages_fetched": local.pages_fetched})()
    else:
    ...
    if trusted_mode and not execute_booking:
        window = await engine.refresh_trusted([snapshot.source_url for snapshot in normalized], matches, upcoming.events)
