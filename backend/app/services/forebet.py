...
def _active_clearance_cookies() -> list[dict]:
    ...
    with _clearance_lock:
        return [
            dict(cookie)
            for cookie in _clearance_cookies
...
def _remember_clearance_cookies(cookies: list[dict]) -> None:
    retained = [
        ...
        if cookie.get("name") in {"cf_clearance", "__cf_bm"}
    ]
    logger.warning(
        "clearance_capture cookies=%s",
        [{"name": c.get("name"), "expires": c.get("expires")} for c in retained],
    )
    if retained:
        with _clearance_lock:
...
def _parse_prediction_result(value: str | None) -> ForebetPredictionResult:
    return {
        "1": ForebetPredictionResult.HOME,
        "X": ForebetPredictionResult.DRAW,
        "2": ForebetPredictionResult.AWAY,
    }.get((value or "").strip().upper(), ForebetPredictionResult.UNKNOWN)
...
def _parse_competition(onclick: str | None) -> tuple[str | None, str | None]:
    if not onclick:
        return None, None
    args = re.search(
        r"getstag\s*\(\s*this\s*,\s*[^,]+,\s*(['\"])(.*?)\1\s*,\s*(['\"])(.*?)\3",
        onclick,
    )
...
def parse_forebet_html(html: str, source_url: str | None = None) -> list[ForebetMatch]:
    ...
    for row in soup.select(".schema > .rcnt"):
        ...
        competition, country = _parse_competition(
            league_img.get("onclick") if league_img else None
        )
        probabilities = row.select(".fprc > span")
        probability_model = None
        if len(probabilities) >= 3:
            probability_model = ForebetProbability(
                home=_parse_float(_text(probabilities[0])),
                draw=_parse_float(_text(probabilities[1])),
                away=_parse_float(_text(probabilities[2])),
            )
        score_home, score_away = _parse_score(
            _text(row.select_one(".predict .scrmobpred"))
        )
        ...
        matches.append(
            ForebetMatch(
                ...
                match_url=urljoin(
                    source_url or get_settings().forebet_base_url,
                    match_href.get("href"),
                )
                if match_href and match_href.get("href")
                else None,
                predicted_result=_parse_prediction_result(
                    _text(row.select_one(".predict .forepr"))
                ),
                ...
                primary_coefficient=_parse_float(
                    _text(row.select_one(".prmod > .lscrsp"))
                ),
                ...
                narrative=_text(narrative_node),
...
def rank_draw_matches(
    matches: Iterable[ForebetMatch], limit: int | None = None
) -> list[ForebetMatch]:
    ...
    ranked = sorted(
        enumerate(draws),
        key=lambda item: (
            item[1].probabilities.draw
            if item[1].probabilities and item[1].probabilities.draw is not None
            else float("-inf"),
            -item[0],
        ),
        reverse=True,
    ...
    return (
        [match for _, match in ranked]
        if limit is None
        else [match for _, match in ranked[:limit]]
    )
...
def _validate_forebet_html(html: str, content_type: str | None) -> None:
    if content_type and "html" not in content_type.lower():
        raise ForebetAcquisitionError(
            f"Forebet returned unsupported content type: {content_type}"
        )
    lowered = html.lower()
    if not html.strip() or "<html" not in lowered:
        raise ForebetAcquisitionError(
            "Forebet returned an empty or malformed HTML response"
        )


def _validate_browser_forebet_html(
    html: str, final_url: str, expected_host: str
) -> None:
    from urllib.parse import urlparse

    if urlparse(final_url).hostname != expected_host:
        raise ForebetAcquisitionError(
            "Forebet browser navigation left the configured domain"
        )
    lowered = html.lower()
    challenge_markers = (
        "captcha",
    ...
    if any(marker in lowered for marker in challenge_markers):
        raise ForebetBrowserChallengeError(
            "Forebet presented an access-denied or CAPTCHA challenge"
        )
    _validate_forebet_html(html, "text/html")
    if "forebet" not in lowered:
        raise ForebetAcquisitionError(
            "Browser response was not recognizable Forebet HTML"
        )
    ...
    if not re.search(r'class=["\'][^"\']*schema[^"\']*["\']', html, re.IGNORECASE):
        raise ForebetBrowserChallengeError(
            "Forebet browser response contained no fixture table"
        )
    ...
    *,
    ...
    if final_host != expected_host:
        ...
        if challenge_indicator:
            ...
            r'class=["\'][^"\']*schema[^"\']*["\']', html, re.IGNORECASE
        ):
...
def _fetch_forebet_page_browser_sync(url: str) -> str:
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            logger.warning(
                "clearance_injection_before url=%s cookies=%s",
                url,
                [
                    {"name": c.get("name"), "expires": c.get("expires")}
                    for c in prior_cookies
                ],
            )
            if prior_cookies:
                context.add_cookies(prior_cookies)
                logger.warning(
                    "clearance_injection_after url=%s names=%s",
                    url,
                    [
                        c.get("name")
            ...
            def _log_request(request):
                if request.is_navigation_request():
                    logger.warning(
                        "browser_navigation_request url=%s referer=%r",
                        request.url,
            ...
            try:
                ...
                if response.status_code == 403:
                    ...
                    if not settings.forebet_browser_fallback_enabled:
                        raise ForebetAccessDeniedError(
                            "Forebet rejected the request with HTTP 403"
                        )
                    try:
                        return await fetch_forebet_page_browser(url)
                    except ForebetAcquisitionError as exc:
                        raise ForebetAccessDeniedError(
                            f"Forebet HTTP 403 and browser fallback failed: {exc}"
                        ) from exc
                response.raise_for_status()
                _validate_forebet_html(
                ...
                if (
                    ...
                    or attempt >= settings.forebet_retries
                ):
                    raise
                await asyncio.sleep(settings.forebet_retry_backoff * (2**attempt))
        raise ForebetAcquisitionError(
            "Forebet acquisition exhausted without a response"
        )
    finally:
        if owns_client:
