...
| POST | `/api/ng/factsCenter/wapConfigurableEventsByOrder` | JSON `{productId:3,sportId:"sr:sport:1",order:0,pageNum,pageSize,withTwoUpMarket:true,withOneUpMarket:true}` with WAP client headers | `{bizCode,message,data:{moreEvents,tournaments[]}}`; same event parser shape, with `moreEvents` controlling pagination. Used only after the desktop endpoint fails. | No credential added. | Yes: fallback, pagination, parser, and request-contract tests. |
...
`get_upcoming_football_events` fetches pages sequentially, up to configured `max_pages` (default 20), stopping when an empty page is returned or `page * pageSize >= totalNum`. The WAP fallback instead follows `data.moreEvents` and records the retrieved count as its total. It records retrieval time, expected total, retrieved total, page count, and a completeness flag. The configured TTL is 300 seconds. Filtering by the requested UTC interval happens after page retrieval, so either endpoint is structurally suitable only when the complete catalog is obtained and fresh.
