...
describe('DashboardPage Phase 3 ticket flow', () => {
  ...
  it('renders Splitter revealed games through the Optimizer game display', async () => {
    const user = userEvent.setup()
    const splitBooking: BookingResponse = { ...original, booking_code: 'SPLIT1', total_selections: 2, selections: [original.selections[0], original.selections[1]] }
    vi.mocked(api.removeSelectedGames).mockResolvedValue(splitBooking)
    render(<DashboardPage />)
    await loadTicket(user)
    await user.click(screen.getByRole('button', { name: 'Open Splitter' }))
    await user.click(screen.getByRole('button', { name: 'First' }))
    await user.clear(screen.getByLabelText('Number of games for splitter 0'))
    await user.type(screen.getByLabelText('Number of games for splitter 0'), '2')
    await user.click(screen.getByRole('button', { name: 'Confirm' }))
    await screen.findByText('SPLIT1')
    await user.click(screen.getByRole('button', { name: 'Toggle split games' }))
    ...
    expect(screen.getByRole('button', { name: 'Select all games on AUGUST 13, 2026' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Remove Selected' })).toBeDisabled()
    expect(screen.getByTestId('selection-control-A')).toBeInTheDocument()
  })

  it('swipe-deletes a Splitter game through remove-selected and updates the regenerated result', async () => {
    const user = userEvent.setup()
    const splitBooking: BookingResponse = { ...original, booking_code: 'SPLIT1', total_selections: 2, selections: [original.selections[0], original.selections[1]] }
    const regenerated: BookingResponse = { ...original, booking_code: 'SPLIT2', total_selections: 1, selections: [original.selections[1]] }
    vi.mocked(api.removeSelectedGames).mockResolvedValueOnce(splitBooking).mockResolvedValueOnce(regenerated)
    render(<DashboardPage />)
    await loadTicket(user)
    await user.click(screen.getByRole('button', { name: 'Open Splitter' }))
    await user.click(screen.getByRole('button', { name: 'First' }))
    ...
    await screen.findByText('SPLIT1')
    await user.click(screen.getByRole('button', { name: 'Toggle split games' }))
    const surface = screen.getAllByTestId('selection-control-A')[0].closest('[data-swipe-delete]') as HTMLElement
    fireEvent.pointerDown(surface, { pointerId: 1, clientX: 0, clientY: 0 })
    fireEvent.pointerMove(surface, { pointerId: 1, clientX: 140, clientY: 0 })
    fireEvent.pointerUp(surface, { pointerId: 1, clientX: 140, clientY: 0 })
    await screen.findByText('SPLIT2')
    expect(api.removeSelectedGames).toHaveBeenLastCalledWith('SPLIT1', ['A'])
    expect(screen.queryByText(/Team A.*Team B/)).not.toBeInTheDocument()
    expect(screen.getByText(/Team C.*Team D/)).toBeInTheDocument()
  })

  it('shows Splitter swipe regeneration loading and preserves the card on failure', async () => {
    const user = userEvent.setup()
    const splitBooking: BookingResponse = { ...original, booking_code: 'SPLIT1', total_selections: 2, selections: [original.selections[0], original.selections[1]] }
    let rejectRequest: (error: Error) => void = () => undefined
    vi.mocked(api.removeSelectedGames).mockResolvedValueOnce(splitBooking).mockImplementationOnce(() => new Promise((_, reject) => { rejectRequest = reject }))
    render(<DashboardPage />)
    await loadTicket(user)
    await user.click(screen.getByRole('button', { name: 'Open Splitter' }))
    await user.click(screen.getByRole('button', { name: 'First' }))
    ...
    fireEvent.pointerMove(surface, { pointerId: 1, clientX: 140, clientY: 0 })
    fireEvent.pointerUp(surface, { pointerId: 1, clientX: 140, clientY: 0 })
    expect(await screen.findByRole('status', { name: '' })).toHaveTextContent('Updating split...')
    rejectRequest(new Error('SportyBet failed'))
    expect(await screen.findByRole('alert')).toHaveTextContent('SportyBet failed')
    expect(screen.getByText('SPLIT1')).toBeInTheDocument()
    expect(screen.getByText(/Team A.*Team B/)).toBeInTheDocument()
  })
})
