              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex items-center gap-3">
                  <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-surface px-3 py-2 text-sm text-slate-300">
                    ...
                    <span><span className="font-bold text-yellow-300">{new Date(match.kickoff).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</span>{' '}<span>{new Date(match.kickoff).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}</span></span>
                  </div>
                  <Button
