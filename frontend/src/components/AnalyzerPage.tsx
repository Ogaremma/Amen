...
export function AnalyzerPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-5">
    ...
    {state === 'success' && <>{days.length > 0 && <section aria-label="Forebet draw booking window" className="space-y-4">{days.map((day) => <BookingDayCard key={day.prediction_date} day={day} />)}</section>}{days.length === 0 && <Card className="p-6 text-center text-sm text-slate-400">No active Forebet draw prediction days are available yet.</Card>}</>}
  </div>
}
